    ...
    <section>
        <div class="p-4 sm:ml-64">
            <div class="flex flex-col">
                <h1 class="text-2xl lg:text-4xl font-bold mt-4 mb-2">Halaman Pengaduan</h1>
            ...
            <div class="relative overflow-x-auto mt-4 px-4">
                <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                    <div class="flex justify-end items-center gap-2">
                        ...
                        @include('rt.components.modals.artikel.sort')
                        @include('rt.components.modals.artikel.search')
                    </div>
                    @if (session('success'))
                    ...
                    <tbody>
                        @foreach ($pengaduan as $index => $p)
                            <tr class="bg-white text-center border-b dark:bg-gray-800 dark:border-gray-700">
                                ...
                                <td class="px-6 py-4">
                                    @if ($p->status != 'selesai')
                                        <form action="{{ route('pengaduan.selesai.rt', $p->id_pengaduan) }}"
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex justify-center items-center space-x-2">
                                        ...
                                        <form action="{{ route('pengaduan.hapus.rt', $p->id_pengaduan) }}"
                                            method="POST" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                            ...
                            @foreach ($pengaduan as $p)
                                @include('rt.components.modals.pengaduan.detail', ['pengaduan' => $p])
                            @endforeach
                        @endforeach
                    </tbody>
                </table>
